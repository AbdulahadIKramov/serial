...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // async function getRencoure(url) {
  //   const res = await fetch(url)

  //   return await res.json()
  // }

  // getRencoure('http://localhost:3000/menu').then(data => {
  ...
  async function fostData(url, data) {
    const res = await fetch(url, {
      method: 'POST',
      headers: {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', (e) => {
      ...
      fostData(' http://localhost:3000/request', json)
        .then((data) => {
          console.log(data)
          showThenksModal(msg.success)
          statusMessage.remove()
        })
        .catch(() => {
          showThenksModal(msg.failure)
        })
        .finally(() => {
          form.reset()
        })
  ...
  function showThenksModal(massge) {
    ...
    thanksModal.innerHTML = `
         <div class="modal__content">
            ...
            <div class="modal__title">${massge}</div>
         </div>
      `
  ...
  indecators.classList.add('corusel-indecators')
  ...
  // function plusSlides(inx) {
  //   showSlides(slideIndex += inx)
  // }
  // next.addEventListener('click', () => {
  //   plusSlides(1)
  // })
